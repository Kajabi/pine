...
export namespace Components {
  ...
  | string;
    ...
    interface PdsCombobox {
        ...
        "disabled": boolean;
        /**
          * Where to mount the dropdown listbox (`host` or `body`, sometimes called “append to body”). Use `body` inside scrollable containers (for example modals) so the list is not clipped by `overflow: hidden` ancestors. Keyboard focus remains on the trigger via `aria-activedescendant` so modal focus traps continue to work. Custom `empty` and `loading` slots are not supported when `body` is used; the default empty and loading content is shown instead.
          * @default 'host'
         */
...
declare namespace LocalJSX {
    ...
    interface PdsAccordion {
         ...
         */
        "onPdsAccordionToggle"?: (event: PdsAccordionCustomEvent<boolean>) => void;
    }
    interface PdsAlert {
        /**
          * A unique identifier used for the underlying component `id` attribute.
