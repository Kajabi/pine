...
- `closeOnEsc` (default: `true`) - Close when the escape key is pressed
- `closeOnBackdropClick` (default: `true`) - Close when clicking outside the modal

```html
<pds-modal closeOnEsc={false} closeOnBackdropClick={false}>
...
<DocCanvas client:only
  mdxSource={{
    ...
    <div>
      ...
      <PdsModal id="default-modal-1" component-id="default-modal-1" closeOnEsc={true}>
        <div slot="header">
          <PdsBox direction="column" fit padding="md">
            <PdsBox
              alignItems="center"
