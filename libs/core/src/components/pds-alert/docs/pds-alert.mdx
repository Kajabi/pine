...
### When to use
- To provide important feedback or information that requires the user's attention.
- To notify users about system status, success messages, warnings, or errors.
- When an immediate user response or awareness is crucial for a particular state or action.
- For critical updates that need to stand out from other page content.

### When not to use
- For non-essential information that can be displayed inline or in less obtrusive ways (e.g., tooltips, popovers).
- As a primary method of navigation or user flow.
- So frequently that they become disruptive or lead to "alert fatigue," causing users to ignore them.
