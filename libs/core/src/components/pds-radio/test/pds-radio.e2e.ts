...
describe('pds-radio', () => {
  ...
  it('emits "sageRadioChange" event when radio is checked', async () => {
    const page = await newE2EPage();
    await page.setContent('<pds-radio component-id="default" label="Label text" />');

    const radio = await page.find('pds-radio >>> input');
    const eventSpy = await page.spyOnEvent('sageRadioChange');
    await radio.press('Space');

    expect(eventSpy).toHaveReceivedEvent();
  });

  it('does not emit "sageRadioChange" event when radio is clicked and disabled', async () => {
    const page = await newE2EPage();
