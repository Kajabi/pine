...
The `actions` property in the `pds-sortable-item` component determines whether the `sortable-item-actions` slot should be enabled.

The `sortable-item-actions` slot is provided to place content within the `pds-sortable-item__actions` element as children, but this slot is only rendered if the `actions` property is set to `true`.
