...
describe('sage-textarea', () => {
  it('renders', async () => {
    ...
    await page.setContent('<sage-textarea></sage-textarea>');
  ...
  it('submits form', async () => {
    const page = await newE2EPage({
      html: `
        <form>
          <sage-textarea type="submit"></sage-textarea>
        </form>
      `
    });
    ...
    await page.evaluate(() => document.querySelector('sage-textarea').click());
  ...
  it('renders when slot is used', async () => {
    const page = await newE2EPage();
    await page.setContent('<sage-textarea>Test Content</sage-textarea>');
    const component = await page.find('sage-textarea');
  ...
  it('renders toggle of disabled state', async () => {
    ...
    const element = await page.find('sage-textarea >>> button');
