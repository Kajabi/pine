...
### Padding Properties Priority
The Box component provides three padding properties with different levels of specificity:

1. `padding` - Sets padding on all sides (lowest priority)
2. `paddingBlock` - Sets padding on top and bottom (medium priority)
3. `paddingInline` - Sets padding on left and right (medium priority)

When multiple padding properties are set, the more specific properties take precedence. For example:
...
This priority system allows for more granular control over padding while maintaining a predictable behavior. When you need to set different padding values for different sides, use the more specific properties (`paddingBlock` and `paddingInline`) to override the general `padding` property.
