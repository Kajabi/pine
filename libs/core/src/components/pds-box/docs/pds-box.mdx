...
**Important:** The `flex` prop only works when the parent element is another `pds-box` component. It does not work when the parent is `pds-row` or other elements, as those components use different layout systems.
...
**Note:** Remember that these flex properties only work when the immediate parent is a `pds-box` component, not when inside `pds-row` or other containers.
