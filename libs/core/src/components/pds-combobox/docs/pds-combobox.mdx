...
- `dropdownMount`: 'host' | 'body' (default: 'host') — also known as an “append to body” pattern; use `body` inside scrollable modals or other `overflow: hidden` containers so the list is not clipped. When `body` is used, keyboard focus stays on the trigger (via `aria-activedescendant`) so modal focus traps keep working.
