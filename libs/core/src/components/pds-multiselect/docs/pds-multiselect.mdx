import { Meta, Canvas, Controls } from '@storybook/addon-docs/blocks';
import * as stories from '../stories/pds-multiselect.stories.js';

import { DocArgsTable, DocCanvas } from '@pine-ds/doc-components';
import { components } from '../../../../dist/docs.json';

<Meta of={stories} />

# Multiselect

The multiselect component allows users to select multiple options from a searchable dropdown list. It displays the selected count in the trigger and shows a searchable list of options in a dropdown panel.

## Guidelines

### When to use
- When users need to select multiple items from a list
- When the list of options is long and benefits from search/filtering
- For filtering, tagging, or assigning multiple values
- When you want to keep the interface compact with a count-based display

### When not to use
- For single selection - use `pds-combobox` or `pds-select` instead
- For very short lists (3-4 items) - consider using checkboxes directly

### Accessibility
- Implements WAI-ARIA Combobox pattern with multiselect support
- Full keyboard navigation: Arrow keys, Enter, Escape, Tab
- Screen reader announces selected count and available options
- Checkboxes in dropdown provide clear selection state

## Usage Notes

### Trigger Display

The trigger button shows:
- The placeholder text when nothing is selected (e.g., "Select...")
- A count when items are selected (e.g., "3 items")

### Panel Structure

When open, the dropdown panel displays:
1. A search input at the top for filtering options
2. A list of currently selected items (if any)
3. The full list of options with checkboxes

### Working with Labels

Text that appears above the multiselect is the component's label, not a separate `pds-text` component. The `label` prop creates accessible text that associates properly with the trigger.

## Properties

<DocArgsTable componentName="pds-multiselect" docSource={components} />

## Examples

### Default

Basic multiselect with static options.

<DocCanvas
  mdxSource={{
    webComponent: `<pds-multiselect
  component-id="tags-select"
  label="Select Tags"
  placeholder="Select..."
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
  <option value="3">Support</option>
  <option value="4">Engineering</option>
</pds-multiselect>`,
    react: `<PdsMultiselect
  componentId="tags-select"
  label="Select Tags"
  placeholder="Select..."
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
  <option value="3">Support</option>
  <option value="4">Engineering</option>
</PdsMultiselect>`,
  }}
>
  <pds-multiselect
    component-id="tags-select"
    label="Select Tags"
    placeholder="Select..."
  >
    <option value="1">Marketing</option>
    <option value="2">Sales</option>
    <option value="3">Support</option>
    <option value="4">Engineering</option>
  </pds-multiselect>
</DocCanvas>

### With Preselected Values

Use the `value` prop to set initially selected items. The trigger will display the count (e.g., "2 items").

<DocCanvas
  mdxSource={{
    webComponent: `<pds-multiselect
  component-id="departments-select"
  label="Departments"
  placeholder="Select..."
  value='["1", "3"]'
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
  <option value="3">Support</option>
</pds-multiselect>`,
    react: `<PdsMultiselect
  componentId="departments-select"
  label="Departments"
  placeholder="Select..."
  value={['1', '3']}
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
  <option value="3">Support</option>
</PdsMultiselect>`,
  }}
>
  <pds-multiselect
    component-id="departments-select"
    label="Departments"
    placeholder="Select..."
    value='["1", "3"]'
  >
    <option value="1">Marketing</option>
    <option value="2">Sales</option>
    <option value="3">Support</option>
  </pds-multiselect>
</DocCanvas>

### Hidden Selected Items

Use the `hide-selected-items` attribute to hide the selected items summary that appears above the options list in the dropdown. This prevents layout shift when selecting multiple items.

<DocCanvas
  mdxSource={{
    webComponent: `<pds-multiselect
  component-id="hidden-selected-select"
  label="Departments"
  placeholder="Select..."
  hide-selected-items
  value='["1", "3"]'
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
  <option value="3">Support</option>
  <option value="4">Engineering</option>
</pds-multiselect>`,
    react: `<PdsMultiselect
  componentId="hidden-selected-select"
  label="Departments"
  placeholder="Select..."
  hideSelectedItems
  value={['1', '3']}
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
  <option value="3">Support</option>
  <option value="4">Engineering</option>
</PdsMultiselect>`,
  }}
>
  <pds-multiselect
    component-id="hidden-selected-select"
    label="Departments"
    placeholder="Select..."
    hide-selected-items
    value='["1", "3"]'
  >
    <option value="1">Marketing</option>
    <option value="2">Sales</option>
    <option value="3">Support</option>
    <option value="4">Engineering</option>
  </pds-multiselect>
</DocCanvas>

### Max Selections

Limit the number of items that can be selected.

<DocCanvas
  mdxSource={{
    webComponent: `<pds-multiselect
  component-id="limited-select"
  label="Select Up to 3 Tags"
  placeholder="Select..."
  max-selections="3"
>
  <option value="1">Tag A</option>
  <option value="2">Tag B</option>
  <option value="3">Tag C</option>
  <option value="4">Tag D</option>
  <option value="5">Tag E</option>
</pds-multiselect>`,
    react: `<PdsMultiselect
  componentId="limited-select"
  label="Select Up to 3 Tags"
  placeholder="Select..."
  maxSelections={3}
>
  <option value="1">Tag A</option>
  <option value="2">Tag B</option>
  <option value="3">Tag C</option>
  <option value="4">Tag D</option>
  <option value="5">Tag E</option>
</PdsMultiselect>`,
  }}
>
  <pds-multiselect
    component-id="limited-select"
    label="Select Up to 3 Tags"
    placeholder="Select..."
    max-selections="3"
  >
    <option value="1">Tag A</option>
    <option value="2">Tag B</option>
    <option value="3">Tag C</option>
    <option value="4">Tag D</option>
    <option value="5">Tag E</option>
  </pds-multiselect>
</DocCanvas>

### Custom Widths

Use `trigger-width` to control the trigger size and `panel-width` to control the dropdown panel size. `min-width` sets a floor for the panel.

<DocCanvas
  mdxSource={{
    webComponent: `<pds-multiselect
  component-id="widths-select"
  label="Narrow Trigger, Wide Panel"
  placeholder="Select..."
  trigger-width="200px"
  panel-width="320px"
  min-width="250px"
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
  <option value="3">Support</option>
  <option value="4">Engineering</option>
</pds-multiselect>`,
    react: `<PdsMultiselect
  componentId="widths-select"
  label="Narrow Trigger, Wide Panel"
  placeholder="Select..."
  triggerWidth="200px"
  panelWidth="320px"
  minWidth="250px"
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
  <option value="3">Support</option>
  <option value="4">Engineering</option>
</PdsMultiselect>`,
  }}
>
  <pds-multiselect
    component-id="widths-select"
    label="Narrow Trigger, Wide Panel"
    placeholder="Select..."
    trigger-width="200px"
    panel-width="320px"
    min-width="250px"
  >
    <option value="1">Marketing</option>
    <option value="2">Sales</option>
    <option value="3">Support</option>
    <option value="4">Engineering</option>
  </pds-multiselect>
</DocCanvas>

### Async with URL

Fetch options from a remote API endpoint. The component expects responses in the format `{ results: [{ id, text }], totalCount }`.

<DocCanvas
  mdxSource={{
    webComponent: `<pds-multiselect
  component-id="async-select"
  label="Search Products"
  placeholder="Select..."
  async-url="/api/products"
  debounce-ms="500"
/>`,
    react: `<PdsMultiselect
  componentId="async-select"
  label="Search Products"
  placeholder="Select..."
  asyncUrl="/api/products"
  debounceMs={500}
/>`,
  }}
>
  <pds-multiselect
    component-id="async-select"
    label="Search Products"
    placeholder="Select..."
  >
    <option value="1">Product A</option>
    <option value="2">Product B</option>
    <option value="3">Product C</option>
  </pds-multiselect>
</DocCanvas>

> **Note:** The live preview uses static options. In production, options are fetched from the API endpoint.

### Consumer-Managed Async

For complex async scenarios (React Query, custom caching), manage data externally and pass options via prop.

<DocCanvas
  mdxSource={{
    webComponent: `<pds-multiselect
  component-id="consumer-async"
  label="Contacts"
  placeholder="Select..."
  .options="[{id: '1', text: 'Alice'}, {id: '2', text: 'Bob'}]"
  @pdsMultiselectSearch="handleSearch"
  @pdsMultiselectChange="handleChange"
/>`,
    react: `<PdsMultiselect
  componentId="consumer-async"
  label="Contacts"
  placeholder="Select..."
  options={contacts}
  onPdsMultiselectSearch={(e) => setQuery(e.detail.query)}
  onPdsMultiselectChange={(e) => setSelected(e.detail.values)}
/>`,
  }}
>
  <pds-multiselect
    component-id="consumer-async"
    label="Contacts"
    placeholder="Select..."
  >
    <option value="1">Alice Johnson</option>
    <option value="2">Bob Smith</option>
    <option value="3">Charlie Brown</option>
  </pds-multiselect>
</DocCanvas>

> **Note:** The live preview uses static options. In production, pass options via the `options` prop and listen to `pdsMultiselectSearch` events.

### With Error State

Display validation errors with the `errorMessage` prop.

<DocCanvas
  mdxSource={{
    webComponent: `<pds-multiselect
  component-id="error-select"
  label="Required Tags"
  placeholder="Select..."
  error-message="Please select at least one tag."
  invalid
>
  <option value="1">Tag A</option>
  <option value="2">Tag B</option>
</pds-multiselect>`,
    react: `<PdsMultiselect
  componentId="error-select"
  label="Required Tags"
  placeholder="Select..."
  errorMessage="Please select at least one tag."
  invalid
>
  <option value="1">Tag A</option>
  <option value="2">Tag B</option>
</PdsMultiselect>`,
  }}
>
  <pds-multiselect
    component-id="error-select"
    label="Required Tags"
    placeholder="Select..."
    error-message="Please select at least one tag."
    invalid
  >
    <option value="1">Tag A</option>
    <option value="2">Tag B</option>
  </pds-multiselect>
</DocCanvas>

### With Create Option

Enable users to create new options when no matches are found by setting the `create-url` prop. When a user types a search query that doesn't match any existing options, they'll see an "Add" option that posts to the specified endpoint.

<DocCanvas
  mdxSource={{
    webComponent: `<pds-multiselect
  component-id="tags-create"
  label="Manage Tags"
  placeholder="Select or create tags..."
  create-url="/api/tags"
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
  <option value="3">Support</option>
</pds-multiselect>`,
    react: `<PdsMultiselect
  componentId="tags-create"
  label="Manage Tags"
  placeholder="Select or create tags..."
  createUrl="/api/tags"
  onPdsMultiselectCreate={(e) => console.log('Created:', e.detail.newOption)}
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
  <option value="3">Support</option>
</PdsMultiselect>`,
  }}
>
  <pds-multiselect
    component-id="tags-create"
    label="Manage Tags"
    placeholder="Select or create tags..."
    create-url="/api/tags"
  >
    <option value="1">Marketing</option>
    <option value="2">Sales</option>
    <option value="3">Support</option>
  </pds-multiselect>
</DocCanvas>

> **Note:** The live preview uses static options. In production with `create-url` set, typing a query with no matches shows "Add 'query'" option.

#### Create Option API Contract

When the create option is clicked, the component sends a POST request:

**Request:**
```json
POST /api/tags
Content-Type: application/json

{
  "text": "New Tag Name"
}
```

**Expected Response:**
```json
{
  "id": "4",
  "text": "New Tag Name"
}
```

The newly created option is automatically added to the list and selected. The component emits:
- `pdsMultiselectCreate` - Contains the query and new option details
- `pdsMultiselectChange` - Updated selection including the new option

**Error Handling:** If the create request fails, an error is logged to the console and the component remains in a usable state. Consider implementing your own error handling by monitoring the `pdsMultiselectCreate` event or network requests.

## CSRF Token Support

When making API requests (async fetching or creating new options), you can configure CSRF token support to include authentication tokens in request headers.

### Configuration

Use the `csrf-token` and `csrf-header-name` props to configure CSRF protection:

<DocCanvas
  mdxSource={{
    webComponent: `<pds-multiselect
  component-id="secure-select"
  label="Select Tags"
  placeholder="Select..."
  async-url="/api/tags"
  create-url="/api/tags"
  csrf-token="abc123xyz"
  csrf-header-name="X-CSRF-Token"
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
</pds-multiselect>`,
    react: `<PdsMultiselect
  componentId="secure-select"
  label="Select Tags"
  placeholder="Select..."
  asyncUrl="/api/tags"
  createUrl="/api/tags"
  csrfToken="abc123xyz"
  csrfHeaderName="X-CSRF-Token"
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
</PdsMultiselect>`,
  }}
>
  <pds-multiselect
    component-id="secure-select"
    label="Select Tags"
    placeholder="Select..."
    csrf-token="abc123xyz"
    csrf-header-name="X-CSRF-Token"
  >
    <option value="1">Marketing</option>
    <option value="2">Sales</option>
  </pds-multiselect>
</DocCanvas>

### How It Works

When both `csrf-token` and `csrf-header-name` are provided:
- All async fetch requests (GET) include the CSRF token in the specified header
- All create requests (POST) include the CSRF token in the specified header
- The token is automatically included in all API calls made by the component

**Request Example with CSRF Token:**
```http
POST /api/tags
Content-Type: application/json
X-CSRF-Token: abc123xyz

{
  "text": "New Tag Name"
}
```

### Common Header Names

Different frameworks use different CSRF header conventions:
- Rails: `X-CSRF-Token`
- Django: `X-CSRFToken`
- Express (csurf): `X-CSRF-Token` or `X-XSRF-Token`

Consult your backend framework's documentation for the correct header name.

### Retrieving CSRF Tokens

Common approaches to get the CSRF token:
- Read from a meta tag: `document.querySelector('meta[name="csrf-token"]')?.content`
- Read from a cookie (for cookie-based CSRF)
- Retrieve from your application's authentication state

**React Example:**
```tsx
const csrfToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content');

<PdsMultiselect
  componentId="secure-tags"
  csrfToken={csrfToken}
  csrfHeaderName="X-CSRF-Token"
  asyncUrl="/api/tags"
/>
```

### Custom Search Placeholder

Override the default "Find..." placeholder text in the search input with the `search-placeholder` prop.

<DocCanvas
  mdxSource={{
    webComponent: `<pds-multiselect
  component-id="search-placeholder-select"
  label="Filter Offers"
  placeholder="Select..."
  search-placeholder="Search offers..."
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
  <option value="3">Support</option>
  <option value="4">Engineering</option>
</pds-multiselect>`,
    react: `<PdsMultiselect
  componentId="search-placeholder-select"
  label="Filter Offers"
  placeholder="Select..."
  searchPlaceholder="Search offers..."
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
  <option value="3">Support</option>
  <option value="4">Engineering</option>
</PdsMultiselect>`,
  }}
>
  <pds-multiselect
    component-id="search-placeholder-select"
    label="Filter Offers"
    placeholder="Select..."
    search-placeholder="Search offers..."
  >
    <option value="1">Marketing</option>
    <option value="2">Sales</option>
    <option value="3">Support</option>
    <option value="4">Engineering</option>
  </pds-multiselect>
</DocCanvas>

### Close Panel on Select

Use `close-panel-on-select` to automatically close the dropdown after each selection. Useful when users typically pick one item at a time and reopen to add more.

<DocCanvas
  mdxSource={{
    webComponent: `<pds-multiselect
  component-id="close-on-select"
  label="Quick Tags"
  placeholder="Select..."
  close-panel-on-select
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
  <option value="3">Support</option>
  <option value="4">Engineering</option>
</pds-multiselect>`,
    react: `<PdsMultiselect
  componentId="close-on-select"
  label="Quick Tags"
  placeholder="Select..."
  closePanelOnSelect
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
  <option value="3">Support</option>
  <option value="4">Engineering</option>
</PdsMultiselect>`,
  }}
>
  <pds-multiselect
    component-id="close-on-select"
    label="Quick Tags"
    placeholder="Select..."
    close-panel-on-select
  >
    <option value="1">Marketing</option>
    <option value="2">Sales</option>
    <option value="3">Support</option>
    <option value="4">Engineering</option>
  </pds-multiselect>
</DocCanvas>

### Dismiss Event

The `pdsMultiselectDismiss` event fires when the dropdown is closed via the Escape key or by clicking outside the component. This is useful for restoring parent UI state or running side effects when the user explicitly dismisses the panel.

<DocCanvas
  mdxSource={{
    webComponent: `<pds-multiselect
  component-id="dismiss-example"
  label="Tags"
  placeholder="Select..."
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
</pds-multiselect>

<script>
  document.querySelector('#dismiss-example')
    .addEventListener('pdsMultiselectDismiss', () => {
      console.log('Panel dismissed');
    });
</script>`,
    react: `<PdsMultiselect
  componentId="dismiss-example"
  label="Tags"
  placeholder="Select..."
  onPdsMultiselectDismiss={() => console.log('Panel dismissed')}
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
</PdsMultiselect>`,
  }}
>
  <pds-multiselect
    component-id="dismiss-example"
    label="Tags"
    placeholder="Select..."
  >
    <option value="1">Marketing</option>
    <option value="2">Sales</option>
  </pds-multiselect>
</DocCanvas>

> **Note:** Open the console, then press Escape or click outside while the dropdown is open to see the dismiss event logged.

### Programmatic Clear

Call the `clear()` method to programmatically reset all selections, clear the search query, and emit a change event with empty values. This is useful for "reset" buttons or when a parent form is cleared.

<DocCanvas
  mdxSource={{
    webComponent: `<pds-multiselect
  id="clearable-select"
  component-id="clearable-select"
  label="Tags"
  placeholder="Select..."
  value='["1", "2"]'
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
  <option value="3">Support</option>
</pds-multiselect>

<pds-button variant="secondary" size="sm" onclick="document.querySelector('#clearable-select').clear()">
  Reset
</pds-button>`,
    react: `const multiselectRef = useRef(null);

<PdsMultiselect
  ref={multiselectRef}
  componentId="clearable-select"
  label="Tags"
  placeholder="Select..."
  value={['1', '2']}
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
  <option value="3">Support</option>
</PdsMultiselect>

<button onClick={() => multiselectRef.current?.clear()}>
  Reset
</button>`,
  }}
>
  <div>
    <pds-multiselect
      id="clearable-select"
      component-id="clearable-select"
      label="Tags"
      placeholder="Select..."
      value='["1", "2"]'
    >
      <option value="1">Marketing</option>
      <option value="2">Sales</option>
      <option value="3">Support</option>
    </pds-multiselect>
    <pds-button style={{ marginTop: 'var(--pine-dimension-sm)' }} variant="secondary" size="sm" onClick={() => {
      const el = document.querySelector('#clearable-select');
      if (el) el.clear();
    }}>Reset</pds-button>
  </div>
</DocCanvas>

### Disabled Options

Individual options can be disabled using the `disabled` attribute on `<option>` elements, or by setting `disabled: true` on items in the `options` prop. Disabled options are rendered as non-interactive and visually muted — they cannot be selected via click or keyboard navigation. Disabling an `<optgroup>` propagates to all its child options.

<DocCanvas
  mdxSource={{
    webComponent: `<pds-multiselect
  component-id="disabled-options-select"
  label="Products"
  placeholder="Select products..."
>
  <option value="ec-1">Classical Guitar Learning Path</option>
  <option value="ec-2" disabled>Piano Lessons (unavailable)</option>
  <option value="ec-3">Music Theory 101</option>
  <option value="cc-1" disabled>Spring Bootcamp 2025 (sold out)</option>
  <option value="cc-2">Fall Workshop Series</option>
</pds-multiselect>`,
    react: `<PdsMultiselect
  componentId="disabled-options-select"
  label="Products"
  placeholder="Select products..."
>
  <option value="ec-1">Classical Guitar Learning Path</option>
  <option value="ec-2" disabled>Piano Lessons (unavailable)</option>
  <option value="ec-3">Music Theory 101</option>
  <option value="cc-1" disabled>Spring Bootcamp 2025 (sold out)</option>
  <option value="cc-2">Fall Workshop Series</option>
</PdsMultiselect>`,
  }}
>
  <pds-multiselect
    component-id="disabled-options-select"
    label="Products"
    placeholder="Select products..."
  >
    <option value="ec-1">Classical Guitar Learning Path</option>
    <option value="ec-2" disabled>Piano Lessons (unavailable)</option>
    <option value="ec-3">Music Theory 101</option>
    <option value="cc-1" disabled>Spring Bootcamp 2025 (sold out)</option>
    <option value="cc-2">Fall Workshop Series</option>
  </pds-multiselect>
</DocCanvas>

### Grouped Options

Use `<optgroup>` elements in the slot to organize options under labeled group headers. Groups are rendered as non-interactive uppercase headings that collapse automatically when search filters out all items in a group.

<DocCanvas
  mdxSource={{
    webComponent: `<pds-multiselect
  component-id="grouped-select"
  label="Products"
  placeholder="Select products..."
>
  <optgroup label="Evergreen Courses">
    <option value="ec-1">Classical Guitar Learning Path</option>
    <option value="ec-2">Piano Lessons</option>
    <option value="ec-3">Music Theory 101</option>
  </optgroup>
  <optgroup label="Cohort Courses">
    <option value="cc-1">Spring Bootcamp 2025</option>
    <option value="cc-2">Fall Workshop Series</option>
  </optgroup>
  <optgroup label="Newsletters">
    <option value="nl-1">Weekly Digest</option>
    <option value="nl-2">Product Updates</option>
  </optgroup>
</pds-multiselect>`,
    react: `<PdsMultiselect
  componentId="grouped-select"
  label="Products"
  placeholder="Select products..."
>
  <optgroup label="Evergreen Courses">
    <option value="ec-1">Classical Guitar Learning Path</option>
    <option value="ec-2">Piano Lessons</option>
    <option value="ec-3">Music Theory 101</option>
  </optgroup>
  <optgroup label="Cohort Courses">
    <option value="cc-1">Spring Bootcamp 2025</option>
    <option value="cc-2">Fall Workshop Series</option>
  </optgroup>
  <optgroup label="Newsletters">
    <option value="nl-1">Weekly Digest</option>
    <option value="nl-2">Product Updates</option>
  </optgroup>
</PdsMultiselect>`,
  }}
>
  <pds-multiselect
    component-id="grouped-select"
    label="Products"
    placeholder="Select products..."
  >
    <optgroup label="Evergreen Courses">
      <option value="ec-1">Classical Guitar Learning Path</option>
      <option value="ec-2">Piano Lessons</option>
      <option value="ec-3">Music Theory 101</option>
    </optgroup>
    <optgroup label="Cohort Courses">
      <option value="cc-1">Spring Bootcamp 2025</option>
      <option value="cc-2">Fall Workshop Series</option>
    </optgroup>
    <optgroup label="Newsletters">
      <option value="nl-1">Weekly Digest</option>
      <option value="nl-2">Product Updates</option>
    </optgroup>
  </pds-multiselect>
</DocCanvas>

For dynamic or async data, add a `group` field to each item in the `options` prop instead. Items that share a `group` label must appear **contiguously** in the array; if the same label appears again after other options, a second header is shown (same behavior as native `<optgroup>`).

<DocCanvas
  mdxSource={{
    webComponent: `<pds-multiselect
  component-id="grouped-options-select"
  label="Products"
  placeholder="Select products..."
/>

<script>
  document.querySelector('#grouped-options-select').options = [
    { id: 'ec-1', text: 'Classical Guitar Learning Path', group: 'Evergreen Courses' },
    { id: 'ec-2', text: 'Piano Lessons', group: 'Evergreen Courses' },
    { id: 'nl-1', text: 'Weekly Digest', group: 'Newsletters' },
    { id: 'nl-2', text: 'Product Updates', group: 'Newsletters' },
  ];
</script>`,
    react: `const products = [
  { id: 'ec-1', text: 'Classical Guitar Learning Path', group: 'Evergreen Courses' },
  { id: 'ec-2', text: 'Piano Lessons', group: 'Evergreen Courses' },
  { id: 'nl-1', text: 'Weekly Digest', group: 'Newsletters' },
  { id: 'nl-2', text: 'Product Updates', group: 'Newsletters' },
];

<PdsMultiselect
  componentId="grouped-options-select"
  label="Products"
  placeholder="Select products..."
  options={products}
/>`,
  }}
>
  <pds-multiselect
    component-id="grouped-options-select"
    label="Products"
    placeholder="Select products..."
    options='[{"id":"ec-1","text":"Classical Guitar Learning Path","group":"Evergreen Courses"},{"id":"ec-2","text":"Piano Lessons","group":"Evergreen Courses"},{"id":"nl-1","text":"Weekly Digest","group":"Newsletters"},{"id":"nl-2","text":"Product Updates","group":"Newsletters"}]'
  ></pds-multiselect>
</DocCanvas>

## Selected Display: Pills

Use `selected-display="pill"` to render selected items as removable pill chips instead of a count. Control placement with `pill-position`.

### Inline Pills

Pills appear inside the trigger button alongside a chevron. This keeps the form field compact while showing exactly what's selected.

<DocCanvas
  mdxSource={{
    webComponent: `<pds-multiselect
  component-id="pill-inline"
  label="Departments"
  placeholder="Select..."
  selected-display="pill"
  pill-position="inline"
  value='["1", "2", "3"]'
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
  <option value="3">Support</option>
  <option value="4">Engineering</option>
  <option value="5">Design</option>
</pds-multiselect>`,
    react: `<PdsMultiselect
  componentId="pill-inline"
  label="Departments"
  placeholder="Select..."
  selectedDisplay="pill"
  pillPosition="inline"
  value={['1', '2', '3']}
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
  <option value="3">Support</option>
  <option value="4">Engineering</option>
  <option value="5">Design</option>
</PdsMultiselect>`,
  }}
>
  <pds-multiselect
    component-id="pill-inline"
    label="Departments"
    placeholder="Select..."
    selected-display="pill"
    pill-position="inline"
    value='["1", "2", "3"]'
  >
    <option value="1">Marketing</option>
    <option value="2">Sales</option>
    <option value="3">Support</option>
    <option value="4">Engineering</option>
    <option value="5">Design</option>
  </pds-multiselect>
</DocCanvas>

### Pills Below

Pills render below the trigger in a wrapping row. Useful when you want the trigger to stay a fixed height and show all selections at a glance.

<DocCanvas
  mdxSource={{
    webComponent: `<pds-multiselect
  component-id="pill-below"
  label="Departments"
  placeholder="Select..."
  selected-display="pill"
  pill-position="below"
  value='["1", "2", "3", "4"]'
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
  <option value="3">Support</option>
  <option value="4">Engineering</option>
  <option value="5">Design</option>
</pds-multiselect>`,
    react: `<PdsMultiselect
  componentId="pill-below"
  label="Departments"
  placeholder="Select..."
  selectedDisplay="pill"
  pillPosition="below"
  value={['1', '2', '3', '4']}
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
  <option value="3">Support</option>
  <option value="4">Engineering</option>
  <option value="5">Design</option>
</PdsMultiselect>`,
  }}
>
  <pds-multiselect
    component-id="pill-below"
    label="Departments"
    placeholder="Select..."
    selected-display="pill"
    pill-position="below"
    value='["1", "2", "3", "4"]'
  >
    <option value="1">Marketing</option>
    <option value="2">Sales</option>
    <option value="3">Support</option>
    <option value="4">Engineering</option>
    <option value="5">Design</option>
  </pds-multiselect>
</DocCanvas>

### Inline Pill Overflow

Use `max-inline-pills` to cap how many chips are shown inline. Selections beyond the limit collapse into a "+N more" badge.

<DocCanvas
  mdxSource={{
    webComponent: `<pds-multiselect
  component-id="pill-overflow"
  label="Departments (max 2 inline)"
  placeholder="Select..."
  selected-display="pill"
  pill-position="inline"
  max-inline-pills="2"
  value='["1", "2", "3", "4", "5"]'
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
  <option value="3">Support</option>
  <option value="4">Engineering</option>
  <option value="5">Design</option>
</pds-multiselect>`,
    react: `<PdsMultiselect
  componentId="pill-overflow"
  label="Departments (max 2 inline)"
  placeholder="Select..."
  selectedDisplay="pill"
  pillPosition="inline"
  maxInlinePills={2}
  value={['1', '2', '3', '4', '5']}
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
  <option value="3">Support</option>
  <option value="4">Engineering</option>
  <option value="5">Design</option>
</PdsMultiselect>`,
  }}
>
  <pds-multiselect
    component-id="pill-overflow"
    label="Departments (max 2 inline)"
    placeholder="Select..."
    selected-display="pill"
    pill-position="inline"
    max-inline-pills="2"
    value='["1", "2", "3", "4", "5"]'
  >
    <option value="1">Marketing</option>
    <option value="2">Sales</option>
    <option value="3">Support</option>
    <option value="4">Engineering</option>
    <option value="5">Design</option>
  </pds-multiselect>
</DocCanvas>

### Disabled Pills

In the disabled state, pills render without close buttons and the trigger is non-interactive.

<DocCanvas
  mdxSource={{
    webComponent: `<pds-multiselect
  component-id="pill-disabled"
  label="Disabled Pill Multiselect"
  placeholder="Cannot select..."
  selected-display="pill"
  pill-position="inline"
  disabled
  value='["1", "2"]'
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
  <option value="3">Support</option>
</pds-multiselect>`,
    react: `<PdsMultiselect
  componentId="pill-disabled"
  label="Disabled Pill Multiselect"
  placeholder="Cannot select..."
  selectedDisplay="pill"
  pillPosition="inline"
  disabled
  value={['1', '2']}
>
  <option value="1">Marketing</option>
  <option value="2">Sales</option>
  <option value="3">Support</option>
</PdsMultiselect>`,
  }}
>
  <pds-multiselect
    component-id="pill-disabled"
    label="Disabled Pill Multiselect"
    placeholder="Cannot select..."
    selected-display="pill"
    pill-position="inline"
    disabled
    value='["1", "2"]'
  >
    <option value="1">Marketing</option>
    <option value="2">Sales</option>
    <option value="3">Support</option>
  </pds-multiselect>
</DocCanvas>

## Form Integration

`pds-multiselect` is a form-associated custom element. It participates in form submission using the `name` attribute.

- Selected values are submitted as multiple entries with the same name (native `<select multiple>` behavior)
- Works with `FormData` API
- Supports form reset

## Playground

<Controls of={stories.Default} />
