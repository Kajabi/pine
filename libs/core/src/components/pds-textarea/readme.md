...
| `characterCountLabel`      | `character-count-label` | Template for the character-counter's accessible label. Pass a translated string to localize it; `{current}` and `{max}` are interpolated.                                                                                                   | `string`  | `'{current} of {max} characters'` |
