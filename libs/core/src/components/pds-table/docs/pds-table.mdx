...
For tables with pagination or large datasets where sorting must be handled server-side, use the `server-side-sorting` attribute. This prevents client-side DOM manipulation and allows your application to handle sorting via server requests or state management.

**When server-side sorting is enabled:**
- Clicking sortable columns dispatches the `pdsTableSort` event without modifying the DOM
- Your application handles the sort request (e.g., API call, database query)
- Update the table data and provide the current sort state via `default-sort-column` and `default-sort-direction`

**Rails/Hotwire Example:**
```erb
<turbo-frame id="products_table">
  <pds-table 
    component-id="products" 
    server-side-sorting
    <% if params[:sort].present? %>
      default-sort-column="<%= column_label_for(params[:sort]) %>"
      default-sort-direction="<%= params[:direction] || 'asc' %>"
    <% end %>
  >
    <pds-table-head>
      <pds-table-head-cell sortable>Name</pds-table-head-cell>
      <pds-table-head-cell sortable>Members</pds-table-head-cell>
      <pds-table-head-cell sortable>Created</pds-table-head-cell>
    </pds-table-head>
    <pds-table-body>
      <% @products.each do |product| %>
        <pds-table-row>
          <pds-table-cell><%= product.name %></pds-table-cell>
          <pds-table-cell><%= product.members_count %></pds-table-cell>
          <pds-table-cell><%= product.created_at.to_date %></pds-table-cell>
        </pds-table-row>
      <% end %>
    </pds-table-body>
  </pds-table>
...
<script>
  document.addEventListener('pdsTableSort', (event) => {
    ...
    url.searchParams.set('sort', columnKeyFor(column));
    url.searchParams.set('direction', direction);
    url.searchParams.delete('page'); // Reset to page 1
    
    // Turbo Frame will fetch sorted data
    document.getElementById('products_table').src = url.toString();
  });
</script>
```
...
function ServerSideTable() {
  ...
  // Load data when sort changes
  useEffect(() => {
    fetch(`/api/products?sort=${sortColumn}&direction=${sortDirection}`)
      .then(res => res.json())
      .then(setData);
  }, [sortColumn, sortDirection]);

  const handleSort = (event) => {
    setSortColumn(event.detail.column);
  ...
  return (
    <pds-table 
      component-id="products"
      server-side-sorting
      default-sort-column={sortColumn}
    ...
    >
      ...
      <pds-table-body>
        {data.map(item => (
          <pds-table-row key={item.id}>
            <pds-table-cell>{item.name}</pds-table-cell>
            <pds-table-cell>{item.email}</pds-table-cell>
          </pds-table-row>
        ))}
      </pds-table-body>
    </pds-table>
...
>
  ...
  <pds-table-body id="table-body">
    <!-- Rows populated dynamically -->
  </pds-table-body>
</pds-table>

<script>
  const table = document.querySelector('pds-table');
  
  table.addEventListener('pdsTableSort', async (event) => {
    const { column, direction } = event.detail;
    
    // Fetch sorted data from server
    const response = await fetch(`/api/users?sort=${column}&dir=${direction}`);
    ...
    tbody.innerHTML = users.map(user => `
      <pds-table-row>
        <pds-table-cell>${user.name}</pds-table-cell>
        <pds-table-cell>${user.email}</pds-table-cell>
      </pds-table-row>
    `).join('');
    
    // Update sort indicators
    table.defaultSortColumn = column;
    table.defaultSortDirection = direction;
  });
</script>
```
