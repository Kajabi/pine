...
| `serverSideSorting`        | `server-side-sorting`    | Enables server-side sorting mode. When enabled, clicking sortable columns will only dispatch the `pdsTableSort` event without performing client-side DOM manipulation. The consuming application is responsible for handling sort logic and re-rendering the table with sorted data via server request or state management. Use `defaultSortColumn` and `defaultSortDirection` to display the correct sort indicator. | `boolean`         | `false`     |
