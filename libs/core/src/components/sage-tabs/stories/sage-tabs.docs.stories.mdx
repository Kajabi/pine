...
<Meta
  ...
  component="sage-tabs"
  argTypes={extractArgTypes('sage-tabs')}
  subcomponents={{
    SageTab: 'sage-tab',
    SageTabPanel: 'sage-tabpanel',
  }}
/>
...
A subcomponent which handles tab logic. This subcomponent is expected to be used exclusively within `sage-tab`.

Requires a unique `name` property to be passed, e.g. `<sage-tab name="foo">Tab Content</sage-tab>`

### Tabpanel

A subcomponent which handles tabpanel logic. This subcomponent is expected to be used exclusively within `sage-tab`.

Requires a matching `name` property to be passed, e.g. `<sage-tabpanel name="foo">Tabpanel Content</sage-tabpanel>`
