...
# Layout
The Layout system is driven by `pds-layout-box`s and `pds-layout-row`s. 
...
#### In order to use the `pds-box` as a grid column, it **MUST** be a direct child of a `pds-row` have a `size` property set.

## `pds-layout-row`
The `pds-layout-row` component is designed to create horizontal rows that contain a set of columns that are capable of altering sizes at different breakpoints. `pds-box` that don't exist in `pds-row` will not container responsive functionality.
