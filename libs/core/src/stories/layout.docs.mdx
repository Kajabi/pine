...
## Row
The `pds-layout-row` component is designed to create horizontal rows that contain a set of columns that are capable of altering sizes at different breakpoints.

## Box
The `pds-layout` serves 2 main purposes:
1. A fully customizable container for your content that allows customization of border, padding, background color, etc.
2. A building block for creating responsive columns, with use of `size` and `offset` properties, when couple with a `pds-layout-row`
